...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            byte[] mosaicBytes = ImageConverter.bufferedImageToBytes(mosaic, "jpg");
            // Print raw byte array preview
            System.out.println("Raw byte array preview:");
            for (int i = 0; i < Math.min(20, mosaicBytes.length); i++) {
                System.out.print(mosaicBytes[i] + " ");
            }
            System.out.println("\nTotal length: " + mosaicBytes.length);

            ImageProcessor processor = new CompressionProcessor();
            byte[] compressed = processor.process(mosaicBytes);
            System.out.println("Compressed mosaic size: " + compressed.length + " bytes");
